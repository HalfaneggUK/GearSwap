...
function check_trust()
	if not moving and state.AutoTrustMode.value and not areas.Cities:contains(world.area) and (buffactive['Reive Mark'] or buffactive['Elvorseal'] or not player.in_combat) then
		...
		if party.p5 == nil then
			...
			if spell_recasts[979] < spell_latency and not have_trust("August") then
				windower.chat.input('/ma "August" <me>')
				tickdelay = (framerate * 4.5)
				return true
			elseif spell_recasts[1012] < spell_latency and not have_trust("Qultada") then
				windower.chat.input('/ma "Qultada" <me>')
				tickdelay = (framerate * 4.5)
				return true
			elseif spell_recasts[1018] < spell_latency and not have_trust("Ulmia") then
				windower.chat.input('/ma "Ulmia" <me>')
				tickdelay = (framerate * 4.5)
				return true
			elseif spell_recasts[1017] < spell_latency and not have_trust("Koru-Moru") then
				windower.chat.input('/ma "Koru-Moru" <me>')
				tickdelay = (framerate * 4.5)
				return true
			elseif spell_recasts[947] < spell_latency and not have_trust("Apururu (UC)") then
